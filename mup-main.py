...
import mup
from mup import MuReadout, make_base_shapes, set_base_shapes, MuAdamW, get_shapes
import subprocess
...
hyp = {
    'opt': {
        ...
        'weight_decay': 0,     # This is the weight decay when the loss = 0., we approach it exponentially. Somewhat slows overfitting.
        ...
        'warmup_steps': 60000000,       # For training stability in the main body of the network. (#TODO: Investigate the warmup imact a bit more)
    },
    'net': {
...
wandb.run.name = f"mup_{hyp['opt']['warmup_steps']}_{model_scale}_{max_sequence_length}"
...
class LatentAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = F.scaled_dot_product_attention(query, key, geglu_attention_value, attn_mask=attn_mask, scale=1./self.qk_dim)
...
def make_net():
    network_dict = nn.ModuleDict({
        ...
        'outputs': MuReadout(hyp['net']['residual_depth'], hyp['misc']['num_tokens'], bias=False),
})
...
infinite_power_law_decay    = lambda tokens_seen, min_initial_mult, peak_tokens, exponent: (
    min_initial_mult + tokens_seen / peak_tokens * (1 - min_initial_mult)
    if tokens_seen < peak_tokens
    else (tokens_seen + 1. - peak_tokens) ** exponent
)
exp_decay_lr_scheduler_base = lambda step, decay: decay ** step

infinite_powah         = partial(infinite_power_law_decay, min_initial_mult=2e-2, peak_tokens=hyp['opt']['warmup_steps'], exponent=-.08)
infinite_powah_outputs = partial(infinite_power_law_decay, min_initial_mult=1.,   peak_tokens=0.,                         exponent=-.2)
pos_bias_decay_lr      = partial(exp_decay_lr_scheduler_base, decay=.995)

# Integrate the new scheduler into the training loop
class TokenBasedLambdaLR(torch.optim.lr_scheduler.LambdaLR):
    def __init__(self, optimizer, lr_lambda, last_epoch=-1):
        self.tokens_seen = 0
        super().__init__(optimizer, lr_lambda, last_epoch)

    def step(self, tokens_seen=None):
        if tokens_seen is not None:
            self.tokens_seen = tokens_seen
        super().step()

    def get_lr(self):
        return [base_lr * lmbda(self.tokens_seen) for lmbda, base_lr in zip(self.lr_lambdas, self.base_lrs)]
...
def init_param_groups_dict(net, base_lr):
    ...
    # Here, we don't scale the learning rate for different groups since we want mup to handle this
    param_groups['decay']                     = {'params': [], 'lr': scaled_lr, 'eps': 1e-9, 'betas': (.9,  .95), 'weight_decay': hyp['opt']['weight_decay'],  'scheduler': infinite_powah}
    param_groups['position_bias_mult']        = {'params': [], 'lr': scaled_lr, 'eps': 1e-9, 'betas': (.9,  .95), 'weight_decay': 0,                           'scheduler': infinite_powah}
    param_groups['norm', 'bias', 'embedding'] = {'params': [], 'lr': scaled_lr, 'eps': 1e-9, 'betas': (.9,  .95), 'weight_decay': 0,                           'scheduler': infinite_powah}
    param_groups['output']                    = {'params': [], 'lr': scaled_lr, 'eps': 1e-9, 'betas': (.9,  .95), 'weight_decay': 0,                           'scheduler': infinite_powah}
...
def main():
    ...
    if args.save_base_shapes:
        hyp['net']['residual_depth'] = 1
        base_shapes = get_shapes(make_net())
        hyp['net']['residual_depth'] = 4
        delta_shapes = get_shapes(make_net())
        make_base_shapes(base_shapes, delta_shapes, args.save_base_shapes)
    ...
    # We use a custom optimizer in order to implement mup
    scaled_lr = 1
    opt = MuAdamW(net.parameters(), lr=scaled_lr, weight_decay=hyp['opt']['weight_decay'], betas=(.9,  .95), fused=True)
    # Get the number of parameter groups
    num_param_groups = len(opt.param_groups)
    lr_lambdas = [infinite_powah] * num_param_groups
    scheduler         = TokenBasedLambdaLR(opt, lr_lambdas)
